...
@php
    ...
    $createdApiKeyBelongsToManager = is_array($createdApiKey)
    ...
    if ($createdApiKeyBelongsToManager && is_string($createdApiKey['encrypted_token'] ?? null)) {
        try {
            $createdApiKeyToken = app(\Illuminate\Contracts\Encryption\Encrypter::class)
                ->decrypt($createdApiKey['encrypted_token'], false);
        } catch (\Throwable) {
            $createdApiKeyToken = null;
        } finally {
    ...
    $createdApiKeyBelongsToManager = $createdApiKeyBelongsToManager
        && is_string($createdApiKeyToken);
    $hasCreationErrors = $errors->has('name')
        || $errors->has('purpose')
        || $errors->has('role')
        || $errors->has('expires_at');
@endphp
